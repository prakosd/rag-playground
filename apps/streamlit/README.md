...
| `rag_shared/resource_cache.py` | App-layer `@st.cache_resource` reuse of heavy RAG clients across turns — cached chat + auxiliary chat models (`cached_chat_resolver` / `cached_aux_resolver`) and one warmed vector searcher per index (`cached_retriever`), injected into `rag_engine` through its `chat_resolver=` / `aux_resolver=` / `retriever=` hooks so the library stays fresh-per-call and unchanged |
| `rag_shared/token_usage_ui.py` | Shared **Token usage** panel renderer for Steps 4-5: `TokenPanelData` + `render_token_usage_panel` (Input / Output / Total / Quota / Usage metrics + a transaction-history table) + `format_cost` |
...
| `focus.py` | Client-side focus helpers: `focus_widget` (focus a keyed input after a rerun, e.g., after a history replay), `entered_page` (tracks page navigation), and `scroll_message_into_view` (pin a keyed message to the top of a scroll container, ChatGPT-style) |
...
| `basic_rag_qa.py` | Step 4 Basic RAG Q&A — index/model/tone/top-results panel, question → **Generate prompt** (`retrieve` + `build_rag_prompt`, template via `basic_rag_qa_form_ui.resolve_basic_rag_qa_prompt_template(session_root)` — a per-session template saved from **Edit template** → `BASIC_RAG_QA_PROMPT_TEMPLATE_FILE` (a customer-service persona by default) → the built-in default; retrieved hits render in an always-present **Search results** panel between the question and prompt via the shared `render_results_panel` (collapsed, titled with the match count, led by the question heading)), editable prompt with an icon-only **Maximize** full-screen dialog with an **Apply** button that writes edits back (dismiss discards; via `basic_rag_qa_form_ui.apply_maximized_prompt`), an **Edit template** editor that saves a per-session prompt template (placeholders validated; `save_`/`reset_basic_rag_qa_template`) → **Send** (`stream_prompt`, streamed answer — in the active UI language — shown in an **Answer** panel inside the prompt form under the button + token/latency stats naming the model as `Model — …`), per-session prompt history (cards with a **:material/info: Metadata** grid (a **Search + answer** timing row shown as `search + answer = total` seconds), a collapsed **Search results** panel, a collapsed **:material/description: Prompt** expander, and a collapsed **:material/forum: Answer** expander with the model/token/latency caption; replay reloads question + prompt + options) and a session token summary above it |
| `conversational_rag.py` | Step 5 conversational RAG — chat UI over the advanced `rag_engine.conversational_answer` pipeline (query decomposition → multi-query retrieval → re-ranking → answer → validated follow-ups → conversation state); the answer and follow-up stages run concurrently, and the grounded answer **streams** token-by-token via `rag_engine.conversational_answer_stream` + `st.write_stream` (its persona mirrors Step 4; the answer follows the active UI language). User turns render as right-aligned (messenger-style) bubbles and the model answer as a matching compact bubble on the left inside a fixed-height **scrollable chat panel** (`st.container(height=…)`, keyed), while the `st.chat_input` renders inline docked under the chat panel and autofocuses on entry and after sending (`focus.entered_page`/`focus_widget` + `focus.scroll_message_into_view`); a live `st.status` shows each stage during generation. Controls: a bordered panel matching Step 4 (Vector index + **Top results**, Answer model + Tone) and an **:material/tune: Advanced options** panel (re-ranking `Off`/`Local`/`LLM` via `st.segmented_control`, a curated **Auxiliary model** selectbox (filtered to the `CONV_RAG_AUX_SIZE_BANDS` small bands via `aux_model_choices`), a follow-up threshold range slider, decomposition / follow-ups / **Inspect mode** toggles, and a tabbed **prompt-template editor** that edits + Saves + Resets each of the LLM-stage prompts (stage tabs hide when their feature is off — Query decomposition / Follow-ups / the LLM re-ranker) plus app-only **Welcome message** / **Error message** / **Follow-up intro** / **No-suggestions** message tabs (one alternate per line, a random one shown per turn; the **Error message** is the natural-language reply shown when a turn fails) — session-saved edit → `config/conversational_*_prompt.txt` → library default). In Inspect mode, a collapsed **:material/search: Inspect** expander shows a **Process | Time·Tokens** grid (two stages per row) + models, then per-stage tabs (Answer / Decomposition / Retrieval / Follow-ups / Conversation state / Diagnostics — the **Answer** tab shows the exact assembled answer prompt, **Retrieval** folds in the re-ranked order and groups results by sub-question, and the last collects a failed turn's technical warnings/errors, kept out of the chat bubble); each LLM-stage tab holds the parsed result plus nested **Prompt** / **Response** sub-tabs showing that stage's exact prompt and raw reply (persisted, so a reloaded turn shows them too), and the benign re-rank / no-follow-up notices surface here rather than beside the answer. Validated follow-ups render **inside the chat panel** as an integrated assistant continuation bubble (avatar hidden) — an editable intro line above the suggestions as link-styled buttons, or a **No-suggestions** nudge when a turn produced none (a cache hit skips retrieval). A conversation control row (a **:material/add:** New button + a picker) groups a session's turns into independent **conversations**: each turn persists to `conversational_rag_history/` (JSONL+CSV, pinnable) tagged with a `conversation_id` + `transaction_id`, so re-opening a session rehydrates the picker and replays a selected conversation's turns from disk (display-adequate — like loading a saved game); the picker titles each conversation from the rewriter's rolling summary (else its first question). A **Token usage** panel (shared `rag_shared/token_usage_ui.py`, also used by Step 4; the Step 5 build via `conversational_token_ui` adds millisecond transaction times, merges a turn's repeated same-process calls into one `N× …` row, and adds Conversation ID / Transaction ID columns) and the standard **Output Files** section render below the chat. Non-UI helpers live in `app_support/conversational_rag/` (`conversation_manager` — short id / title / grouping + display-adequate turn replay, `conversational_rag_form_ui`, `conversational_prompts`, `conversational_token_ui`, `followup_cache`, `conversational_rag_history`) |
...
Either opens an interactive Three.js "universe" of the crawl in a new browser tab: one planet per page (sized by page weight, surfaced by information richness, coloured by crawl status — **failed pages become realistic asteroids** (dark, irregular rocky bodies whose lumpy shape and cratered surface are procedurally generated from each page's URL — a displaced icosphere with a seeded canvas texture — so every failed page looks distinct; they emit no light and stay below the bloom threshold, so the scene's bloom lights only the suns and the trajectory lines) — ringed when large), the
...
timing/direction) while the link is unselected; a **failed page** additionally keeps a **red**
spoke to its parent that carries its own travelling light, so failed pages stand out when you zoom
out (asteroid shape + moving red link). A HUD **Scale** slider (0.2×–5×, default 1×), docked at the
bottom, scales every body live while the orbits stay fixed. Hovering a
planet shows its details and lights the link chain back to the root in **yellow** (with a travelling
light too); clicking lights its
chain in **green** and opens a side panel
