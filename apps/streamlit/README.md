...
| `qa_form_ui.py` | Step 4 pure helpers: `tone_choices` (.env tones + default), `token_totals` + `usage_percent` (session Token count panel: Input / Output / Total / Quota / % Usage), and `apply_maximized_prompt` (Maximize dialog ↔ inline prompt sync; edits autosave on dismiss) |
