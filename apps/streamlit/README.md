...
| `rag_qa.py` | Step 4 Simple RAG Q&A — index/model/tone/top-results panel, question → **Generate prompt** (`retrieve` + `build_rag_prompt`, template from `RAG_QA_PROMPT_TEMPLATE_FILE` via `qa_form_ui.resolve_qa_prompt_template`), editable prompt with an icon-only **Maximize** full-screen dialog (synced via `qa_form_ui.apply_maximized_prompt`) → **Send** (`stream_prompt`, streamed answer shown in an **Answer** panel inside the prompt form under the button + token/latency stats), per-session prompt history with replay and a session token summary above it |
