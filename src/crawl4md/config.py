...
class SessionHeader(BaseModel):
    """First line of ``session.jsonl`` — configs + creation timestamp."""
...
class RoundRecord(BaseModel):
    """Per-round state snapshot appended to ``session.jsonl`` after each round."""
...
class SessionComplete(BaseModel):
    """Optional sentinel at the end of ``session.jsonl`` marking clean completion."""
