...
- **Manifest.** `manifest.py` owns `write_manifest`, `load_manifest`, and `IndexManifest`. `run` writes `manifest.json` recording the requested/used embedding model, dimension, `collection_name`, and `created_at` (ISO-8601 UTC derived from the run-directory timestamp slug via `artifact_store.naming.parse_utc_timestamp_slug` — single source of truth, no fresh `datetime.now`); `IndexManifest.created_at` is `str | None` and `load_manifest` defaults it to `None` for older manifests. Readers use `load_manifest(run_dir)` to reopen an index.
