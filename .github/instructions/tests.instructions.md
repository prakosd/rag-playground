...
- **Never download embedding models.** `vector_indexer` tests must use fake providers, fakes or `pytest.importorskip` for the vector store, and explicit embeddings — the offline embedder downloads ~80 MB on first use.
