...
- **Pure foundation.** `artifact_store` must not import `streamlit`, `crawl4md_streamlit`, `crawl4md`, `crawl4ai`, or `pymupdf`. It uses the standard library plus `log4py` (the logging base layer, itself pure stdlib) only. A boundary test enforces this.
