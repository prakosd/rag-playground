...
- `postAttachCommand` starts Streamlit with `python -m streamlit run streamlit_app.py --server.address=0.0.0.0 --server.port=8501`; keep `0.0.0.0` so forwarded ports work from containers and Codespaces.

## Dependencies

crawl4ai, trafilatura, markdownify, pydantic, nest-asyncio, beautifulsoup4, mdformat + mdformat-gfm, pymupdf4llm, httpx, streamlit. Full list in `pyproject.toml`.
